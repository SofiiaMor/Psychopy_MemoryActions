...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\psychopy\\2D_scenes\\2D_scenes_eng_drawing.py',
...
text_instr2 = visual.TextStim(win=win, name='text_instr2',
    ...
    color=[0, 255, 127], colorSpace='rgb255', opacity=1, 
    languageStyle='LTR',
    depth=-4.0);
...
for thisTrial in trials:
    ...
    for thisLoopPause in loopPause:
        ...
        if condition=='C2':
            textRespInstr='Response by mouse: draw a line from the closest object to the border using the left button of mouse'
        elif condition=='A2' or condition=='B2':
            textRespInstr='Response by mouse: draw a line from the cross to the closest object using the left button of mouse'
        else:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if mouse_resp.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > mouse_resp.tStartRefresh + 2.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if brush.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > brush.tStartRefresh + 1.5-frameTolerance:
                # keep track of stop time/frame for later
    ...
    #SOfiia <-
    brush.reset() #clear the screen from drawing lines
    
    mouse_button = [bool(k) for k in mouse_resp.leftButton] # make a boolean vector(true, false) from a list of 0 and 1, 1 - the button was clicked 
    ind_mouse = [i for i, k in enumerate(mouse_button) if k] # indices of true values
    x = [mouse_resp.x[i] for i in ind_mouse]
    y = [mouse_resp.y[i] for i in ind_mouse]
    mouse_RT = [mouse_resp.time[i] for i in ind_mouse]
    mouse_resp.corr = []                
    ...
    if response=='by_mouse'and sum(mouse_resp.leftButton)>0:
        clicked=1;
        if condition=='C2':
          x = x[1]
          y = y[1]
    ...
    #x = mouse_resp.x[mouse_resp.leftButton(mouse_resp.leftButton>0)]
    #y = mouse_resp.y[mouse_resp.leftButton]
    #mouse_RT = mouse_resp.time[mouse_resp.leftButton]
    ...
    trials.addData('mouse_RT', mouse_RT)
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if text_thanks.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > text_thanks.tStartRefresh + 2.5-frameTolerance:
            # keep track of stop time/frame for later
